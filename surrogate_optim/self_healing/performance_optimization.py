...
class JaxOptimizer:
    ...
    def _configure_jax(self) -> None:
        ...
        if self.config.enable_gpu:
            # Configure GPU memory growth - use proper JAX config option
            try:
                jax.config.update("jax_default_preallocate", False)
            except Exception:
                pass  # Config option might not be available in all JAX versions
